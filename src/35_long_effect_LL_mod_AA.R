...
# Long-term effect - Li-Lee modified auto ARIMA
...
source("fun/li_lee_mod_auto_arima.R")
# Function to apply the Li-Lee (2005) modified with auto ARIMA.
...
#-------------------------------------------------------------------------------
# Obtaining cohort life expectancy for all countries using the Li-Lee mod AA
...
for (c in 1:length(filter_country)) {
  ...
  # 
  # plot(t1,e01.obs-e02.obs,ylim=range(e01.obs-e02.obs,e0.fore1.lm-e0.fore2.lm),
  #xlim=range(t1,tF))
  # abline(h=0)
  # lines(tF,e0.fore1.lm-e0.fore2.lm,col=2);lines(tF,e0.fore2.lm,col=2)
  # lines(tF,e0.fore1.ll-e0.fore2.ll,col=4);lines(tF,e0.fore2.ll,col=4)
  ...
  for (sim in 1:nS) {
    ...
    lmx1_all_obs <- cbind(LMX1, LMX1_fore_sim)
    lmx2_all_obs <- cbind(LMX2, LMX2_fore_sim)
...
#were obtained forecasting with the Li-Lee modified AA using data until 2019.
...
#with the Li-Lee modified with auto ARIMA using data until 2019.
