...
for (c in 1:length(filter_country)) {
  ...
  for (s in 1:length(filter_sex)) {
    ...
    E <- as.matrix(aux_E %>% 
                     ...
                     dplyr::select(-c(age, sex,`2020`,`2021`,`2022`,`2023`)))
    D <- as.matrix(aux_D %>%
                     filter(sex == filter_sex[s]) %>% 
                     dplyr::select(-c(age, sex,`2020`,`2021`,`2022`,`2023`)))
    LMX <- log(as.matrix(aux_MX %>% 
                           filter(sex == filter_sex[s]) %>% 
                           dplyr::select(-c(age, sex,`2020`,
                                            `2021`,`2022`,`2023`))))
    ...
    for (sim in 1:nS) {
      ...
      LMX_obs <- log(as.matrix(aux_MX %>% 
                                 filter(sex == filter_sex[s]) %>% 
                                 dplyr::select(c(`2020`,`2021`,`2022`,`2023`))))
...
write.table(
  ...
  row.names = F,
  "out/23_lmx_short_CPsplines.txt")
