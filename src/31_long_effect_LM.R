...
#-------------------------------------------------------------------------------
# Obtaining cohort life expectancy for all countries using the Lee-Miller
...
for (c in 1:length(filter_country)) {
  ...
  for (s in 1:length(filter_sex)) {
    ...
    LMX_fore <- LM(E=E, LMX=LMX, Y=D, a=a, sex=filter_sex[s],
                   t1=t1, tF=tF, nS=nS)
...
#were obtained forecasting with the Lee-Miller using data until 2023.
...
#forecasting with the Lee-Miller using data until 2023.
