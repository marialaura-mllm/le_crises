...
# Short-term effect - Product Ratio
...
for (c in 1:length(filter_country)) {
  ...
  lmx1_all_obs <- cbind(LMX1, LMX1_obs, LMX1_fore[,5:82])
  lmx2_all_obs <- cbind(LMX2, LMX2_obs, LMX2_fore[,5:82])
...
write.table(
  ...
  row.names = F,
  "out/26_e0_short_product_ratio.txt")
...
write.table(
  ...
  row.names = F,
  "out/26_lmx_short_product_ratio.txt")


# period e0 obs & forecast
save(period_e0,file="out/26_period_e0_product_ratio.Rdata")
