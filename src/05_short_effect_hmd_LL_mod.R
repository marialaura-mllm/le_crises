...
# TThis code is to be used to replicate the results of the paper "Evaluating the 
...
#estimates considering a short-term disturbance using the example of the 
#COVID-19 pandemic and data from HMD.
...
for (c in 1:length(filter_country)) {
  ...
  for (sim in 1:nS) {
    ...
    lmx1_all <- cbind(LMX1_HMD, LMX1_fore_sim)
    lmx2_all <- cbind(LMX2_HMD, LMX2_fore_sim)
    ...
    lmx1_all_obs <- cbind(LMX1_HMD, LMX1_obs, LMX1_fore_sim[,5:82])
    lmx2_all_obs <- cbind(LMX2_HMD, LMX2_obs, LMX2_fore_sim[,5:82])
...
write.table(
  ...
  row.names = F,
  "out/05_e0_short_hmd_LL_mod.txt")
...
write.table(
  ...
  row.names = F,
  "out/05_lmx_short_hmd_LL_mod.txt")
